...
download_libtar() {
    download_file $LIBTAR_LINK $LIBTAR_CHECKSUM ${LIBTAR_PKGNAME}-${LIBTAR_VERSION}.tar.gz
}

extract_libtar() {
    if [ ! -d "${LIBTAR_PKGNAME}-${LIBTAR_VERSION}" ]; then
        progress "extracting: ${LIBTAR_PKGNAME}-${LIBTAR_VERSION}"
        tar -xf ${DISTFILES}/${LIBTAR_PKGNAME}-${LIBTAR_VERSION}.tar.gz -C .
    fi
}

prepare_libtar() {
    progress "configuring: ${LIBTAR_PKGNAME}"
...
compile_libtar() {
    progress "compiling: ${LIBTAR_PKGNAME}"

    make ${MAKEOPTS}
}

install_libtar() {
    progress "installing: ${LIBTAR_PKGNAME}"
