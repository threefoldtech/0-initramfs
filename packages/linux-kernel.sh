...
extract_kernel() {
    if [ ! -d "linux-${KERNEL_VERSION}" ]; then
        progress "extracting: linux-${KERNEL_VERSION}"
        tar -xf ${DISTFILES}/linux-${KERNEL_VERSION}.tar.xz -C .
    fi
}

prepare_kernel() {
    progress "copying kernel configuration"
    ...
    if [ -f arch/x86/mm/init_64.c.orig ]; then
        progress "cleaning previous patch"
    ...
    if [ ! -f .patched_linux-4.9-secureboot-restriction.patch ]; then
        progress "applying (release) boot-restriction patch"
...
compile_kernel() {
    ...
    if [[ $DO_ALL == 1 ]] || [[ $DO_KMODULES == 1 ]]; then
        progress "compiling the kernel (modules)"
        make ARCH=${BUILDARCH} CROSS_COMPILE=${BUILDHOST}- ${MAKEOPTS} modules
        make ARCH=${BUILDARCH} CROSS_COMPILE=${BUILDHOST}- INSTALL_MOD_PATH="${ROOTDIR}" modules_install
        # depmod -a -b "${ROOTDIR}" "${KERNEL_VERSION}-Zero-OS"
    fi

    if [[ $DO_ALL == 1 ]] || [[ $DO_KERNEL == 1 ]]; then
        progress "compiling the kernel (vmlinuz)"
